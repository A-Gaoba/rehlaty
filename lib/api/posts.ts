...
export async function listPosts(cursor?: string, limit = 10, taggedUserId?: string) {
  const qs = new URLSearchParams()
  if (cursor) qs.set('cursor', cursor)
  if (limit) qs.set('limit', String(limit))
  if (taggedUserId) qs.set('taggedUserId', taggedUserId)
...
export async function likePost(id: string) {
  return await apiFetch<{ likesCount: number }>(`/api/posts/${id}/like`, {
    method: 'POST',
  })
}

export async function unlikePost(id: string) {
  return await apiFetch<{ likesCount: number }>(`/api/posts/${id}/unlike`, {
    method: 'POST',
  })
}
