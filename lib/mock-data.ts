import type { User, Post, Comment, Follow, Message, Conversation, Notification, City } from "./types"
...
export const mockUsers: User[] = [
  {
    ...
    avatar: "/arabic-man-profile-photo.jpg",
    bio: "مسافر عربي يحب استكشاف روسيا 🇷🇺 | موسكو ← سانت بطرسبرغ",
    coverPhoto: "/red-square-moscow.jpg",
    isPrivate: false,
    followersCount: 1250,
    followingCount: 890,
    postsCount: 45,
    isVerified: true,
    joinedAt: "2023-01-15",
    interests: ["سياحة", "تصوير", "طعام", "تاريخ"],
  },
  {
    id: "2",
...
export const mockPosts: Post[] = [
  {
    ...
    userId: "1",
    user: mockUsers[0],
    caption: "زيارة مذهلة للكرملين في موسكو! المكان يحكي تاريخ روسيا العريق 🏰 #موسكو #كرملين #سياحة",
  ...
  },
  {
    ...
    userId: "2",
    user: mockUsers[1],
    caption: "متحف الهيرميتاج في سانت بطرسبرغ - كنز من الفن والثقافة! قضيت يوماً كاملاً هنا ولم أشبع 🎨",
    image: "/placeholder.svg?height=400&width=400",
    location: {
...
export const mockFollows: Follow[] = [
  { id: "1", followerId: "1", followingId: "2", status: "accepted", createdAt: "2024-01-01T00:00:00Z" },
  { id: "2", followerId: "1", followingId: "3", status: "accepted", createdAt: "2024-01-02T00:00:00Z" },
  { id: "3", followerId: "2", followingId: "1", status: "accepted", createdAt: "2024-01-03T00:00:00Z" },
  { id: "4", followerId: "2", followingId: "4", status: "accepted", createdAt: "2024-01-04T00:00:00Z" },
  { id: "5", followerId: "3", followingId: "1", status: "pending", createdAt: "2024-01-05T00:00:00Z" },
  { id: "6", followerId: "4", followingId: "6", status: "pending", createdAt: "2024-01-06T00:00:00Z" },
]
...
export const getMockFollowersCount = (userId: string): number => {
  return mockFollows.filter((follow) => follow.followingId === userId && follow.status === "accepted").length
}

export const getMockFollowingCount = (userId: string): number => {
  return mockFollows.filter((follow) => follow.followerId === userId && follow.status === "accepted").length
}

export const isFollowing = (followerId: string, followingId: string): boolean => {
  return mockFollows.some(
    (follow) => follow.followerId === followerId && follow.followingId === followingId && follow.status === "accepted",
  )
}

export const hasPendingFollowRequest = (followerId: string, followingId: string): boolean => {
  return mockFollows.some(
    (follow) => follow.followerId === followerId && follow.followingId === followingId && follow.status === "pending",
  )
}
...
export const getUnreadNotificationsCount = (userId: string): number => {
  return mockNotifications.filter((notification) => notification.userId === userId && !notification.isRead).length
}
