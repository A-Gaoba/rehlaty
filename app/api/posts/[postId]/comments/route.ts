...
export async function GET(req: Request, { params }: { params: { postId: string } }) {
	try {
		...
		const items = await Comment.find(query)
			.sort({ createdAt: -1 })
			.limit(limit + 1)
			.populate({ path: 'userId', model: User, select: 'username displayName isPrivate isVerified createdAt' })
			.lean()
...
const CreateSchema = z.object({ content: z.string().min(1), parentId: z.string().optional() })
...
export async function POST(req: Request, { params }: { params: { postId: string } }) {
	try {
		...
		await Post.updateOne({ _id: params.postId }, { $inc: { commentsCount: 1 } })
		const populated = await created.populate({ path: 'userId', model: User, select: 'username displayName isPrivate isVerified createdAt' })
		return Response.json({ comment: toClientComment(populated) }, { status: 201 })
	} catch (e) {
